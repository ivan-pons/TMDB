...
class MovieRemoteDataSource @Inject constructor(
    ...
    suspend fun getTopFilms(): List<BasicItemDto> {
        return coreApi.getTopFilms().parseResponse().results ?: listOf()
    }

    suspend fun getUpcomingFilms(): List<BasicItemDto> {
        return coreApi.getUpcomingFilms().parseResponse().results ?: listOf()
    }

    suspend fun getNowPlayingFilms(): List<BasicItemDto> {
        return coreApi.getNowPlayingFilms().parseResponse().results ?: listOf()
    }
